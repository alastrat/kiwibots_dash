...
export const MapWrapped = GoogleApiWrapper({
  apiKey: process.env.REACT_APP_API_GOOGLE,
  // LoadingContainer: LoadingContainer
})(({ google, children, mapRef, night, ...props }) => {
  ...
  return (
    <Map
      ...
      {...props}
      styles={cleanedStyle.concat(night ? nightStyle : [])}
    >
      {children}
    </Map>
  );
...
const MapComponent = ({ className, ...props }) => {
  return (
    <div className={clsx(styles["maps-container"], className)}>
      <MapWrapped {...props} />
    </div>
  );
};
